...
bool sd_mount()
{
    ...
    FRESULT fr;
    if (!TULIP_uSD_p->state.mounted) {
        cli_printf("  SDcard is not mounted");
        cli_printf("  mounting SDcard");
        ...
        cli_printf("  SDcard is mounted");
    }
    else
    {
        cli_printf("  SDcard is already mounted");
    }
    return true;
}
...
void sd_unmount()
{
    ...
    cli_printf("  SDCard is unmounted");
}
...
FRESULT sd_dir(const char *dir)
{
    ...
    while (fr == FR_OK && fno.fname[0]) { /* Repeat while an item is found */
        ...
        /* Create a string that includes the file name, the file size and the
         attributes string. */
        cli_printf("%-20s [%s] [%10d]", fno.fname, pcAttrib, fno.fsize);
...
void sd_status() 
{
    ...
    for (size_t i = 0; i < sd_get_num(); ++i) {
        ...
        if (TULIP_uSD_p) {
            cli_printf("  SDcard at drivenum : %d", i);
        } else {
            cli_printf("  no SDcard found at : %d", i);
        }
    }
    ...
    if (!TULIP_uSD_p)
    {
        ...
        cli_printf("  SD card was not initialized, try again");
    ...
    if (STA_NODISK & ds || STA_NOINIT & ds) {
        // cannot initialize
        cli_printf("  SDcard initialization failed");
        return;
    } else {
        cli_printf("  SDcard initialized");
    }
    ...
    if (ok)
        cli_printf("  SDcard Allocation Unit (AU_SIZE) or \"segment\": %zu bytes (%zu sectors)", 
