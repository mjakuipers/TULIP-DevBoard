...
void uif_status()
{
    ...
    cli_printf("*");
    cli_printf("*   VERSION 00.01.01 BETA");
    cli_printf("*");
...
void uif_cdc_ident()
{
  ...
  for (i = 0; i < 5; i++)
  {
    if (cdc_connected(i))
    {
      cli_printf("  CDC Port %d OK  connected: %s\n", i, ITF_str[i]);
...
void uif_bootsel()
{
  ...
  cli_printf("  RESETTING THE TULIP4041 RP2040 to BOOTSEL mode in 2 seconds!! press any key to cancel");
  ...
  while(true)
  {
    ...
    if (sleepcount == 0)
    {  
    // reboots the RP2040 when the counter expires, uses the standard LED for activity monitoring
...
void uif_list()
{
  ...
  if (globsetting.get(ILPRINTER_plugged)) {
      cli_printf("  ILPRINTER    Page 6"); 
  } else {
      cli_printf("  ILPRINTER    Page -");
  }

  if (globsetting.get(HP82143A_enabled)) {
      cli_printf("  PRINTER      Page 6"); 
  } else {
      cli_printf("  PRINTER      Page -");
  }

  if (globsetting.get(HPIL_plugged)) {
      cli_printf("  ILMODULE     Page 7"); 
  } else {
      cli_printf("  ILMODULE     Page -");
  }
...
void uif_printer(int i) {
  ...
  if (!globsetting.get(HP82143A_enabled)) {
    cli_printf("  HP82143A ROM not plugged, settings will not be active until the Printer ROM is plugged");
  }
...
void uif_tracer(int i) {
  ...
  switch (i) {
    case 1: // status
            // cli_printf("  status of HP41 bus tracer", globsetting.get(xmem_pages));
...
void rp2040_sleep()
{
  ...
  printf("TUP4041 will go to sleep\n");
  sleep_ms(1000);
