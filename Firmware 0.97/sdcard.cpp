...
// global variable to keep track of the current working directory, used by the CLI command "cd" and "pwd"
// FatFS does not (yet) support getcwd for the exFAT filesystem
static char current_working_directory[FF_LFN_BUF] = {0};
...
void sdcard_init()
{
    ...
    // initialize the current working directory to root
...
void sd_getcwd(char *cwdbuf, size_t bufsize)
{
    if (current_working_directory[0]) {
        // if we have a current working directory, return it
        strncpy(cwdbuf, current_working_directory, bufsize);
    } else {
        // otherwise, return the root directory
        strncpy(cwdbuf, "/", bufsize);
    }
}

// function to changethe current working directory
FRESULT sd_chdir(const char *path)
{
    ...
    strncpy(current_working_directory, path, sizeof(current_working_directory));
    return FR_OK;
}   
