...
#define FILETYPE_EMPTY              0x00      // indicates an erased file in FLASH
...
#define FILETYPE_XMEM               0x04      // XMEM file
...
#define FILETYPE_ERASED             0x7F      // erased or dummy file in FRAM
...
#define ROM_IMAGE_SIZE 4096     // size of a ROM image in bytes, 4K words or 5120 bytes in packed format
#define MOD_IMAGE_SIZE 5120     // size of a MOD image in bytes, 4K words or 5120 bytes in packed format

// page struct for MOD1
typedef struct {
  byte Image[MOD_IMAGE_SIZE];       // the image in packed format (.BIN file format)
...
typedef struct {
  word Image[ROM_IMAGE_SIZE / 2];       // the image in unpacked format (.ROM file format)
  byte PageCustom[32];    // for special hardware attributes
} V2_t;
...
typedef struct {
  ...
  uint8_t Image[MOD_IMAGE_SIZE];    // the image in packed format (.BIN file format)
  byte PageCustom[32];    // for special hardware attributes
} ModuleHeader_t;

// page struct for module, but without the ROM Image
typedef struct {
  char Name[20];          // normally the name of the original .ROM file, if any
  char ID[9];             // ROM ID code, normally two letters and a number are ID and last letter is revision - if all zeros, will show up as @@@@
  byte Page;              // the page that this image must be in (0-F, although 8-F is not normally used) or defines each page's
...
typedef struct {
  ...
  byte WriteProtect;      // 0=No or N/A, 1=protected - for HEPAX RAM and others that might support it
  byte FAT;               // 0=no FAT, 1=has FAT
  word Image[ROM_IMAGE_SIZE / 2];       // the image in upacked format (.ROM file format)
  byte PageCustom[32];    // for special hardware attributes
} ModuleHeaderV2_t;
...
typedef struct {
  uint16_t RomImage[ROM_IMAGE_SIZE / 2];    // the image in unpacked format (.ROM file format)
} ROMFilePage_t;
