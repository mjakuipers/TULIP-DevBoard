...
#define FRAM_INIT_ADDR          0x80                    // address to store the FRAM initialization value
#define FRAM_INIT_VALUE         0x4041                  // value to indicate that the FRAM is initialized

#define FRAM_EXTDMEM_SIZE       0x1400                  // Extended Functions Memory 0x040 - 0x0BF      128 registers
                                                        // Extended Memory Module #1 0x200 - 0x2FF      256 registers
                                                        // Extended Memory Module #2 0x300 - 0x3FF      256 registers
                                                        // total 640 registers with 8 bytes each = 5120 bytes (0x1400)

#define FRAM_USERMEM_SIZE       0xA80                   // Status Register mirror          0x000 - 0x00F      16 registers
                                                        // standard built-in memory mirror 0x0C0 - 0x0FF      64 registers
                                                        // Quad Memory Module              0x100 - 0x1FF      256 registers
                                                        // total 336 registers with 8 bytes each = 2688 bytes (0xA80)
#define STATUS_REG_SIZE         0x80                    // size of the status register mirror in FRAM, 16 registers with 8 bytes each = 128 bytes (0x80)
#define BUILTIN_MEM_SIZE        0x200                   // size of the built-in memory mirror in FRAM, 64 registers with 8 bytes each = 512 bytes (0x200)
...
#define FRAM_XMEM0_size         0x400                                       // size of XFUN memory module in FRAM, 128 registers with 8 bytes each = 1024 bytes (0x400)

#define FRAM_XMEM1_START        (FRAM_EXTDMEM_CONTENT + FRAM_XMEM0_size)    // start address of XMEM module #1 in FRAM, mapped to 0x200 - 0x2FF in the HP41 system
#define FRAM_XMEM1_size         0x800                                       // size of XMEM module #1 in FRAM, 256 registers with 8 bytes each = 2048 bytes (0x800)

#define FRAM_XMEM2_START        (FRAM_EXTDMEM_CONTENT + FRAM_XMEM0_size + FRAM_XMEM1_size)   // start address of XMEM module #2 in FRAM, mapped to 0x300 - 0x3FF in the HP41 system
#define FRAM_XMEM2_size         0x800                                       // size of XMEM module #2 in FRAM, 256 registers with 8 bytes each = 2048 bytes (0x800)

#define FRAM_STATUS_REG_START   FRAM_USERMEM_CONTENT 
#define FRAM_USERMEM_START      FRAM_USERMEM_CONTENT + STATUS_REG_SIZE
#define QUADMEM_START           FRAM_USERMEM_START + BUILTIN_MEM_SIZE

#define FRAM_FS_CONTENT_START   FRAM_EXTDMEM_CONTENT + FRAM_EXTDMEM_SIZE     // start of file system contents

// below are the hard addresses of the offsets in FRAM for the different files
// these are used for quick access to the files without having to search for them by name
#define    FRAM_SETTINGS_start       0x00128
#define    FRAM_ROMMAP_start         0x00352
#define    FRAM_TRACER_start         0x012BA
#define    FRAM_STATUS_REGS_start    0x052E2
#define    FRAM_USERMEM_start        0x05362
#define    FRAM_QUADMEM_start        0x05562
#define    FRAM_XMEM0_start          0x05D8A
#define    FRAM_XMEM1_start          0x0618A
#define    FRAM_XMEM2_start          0x0698A
