...
uint64_t umem_cache[16];                    // user memory cache, to be refreshed when there is a matching RAMSLCT
uint64_t umem_cache_slct;                   // User memory cache of the RAMSLCT'd register
...
// extern queue_t PowerBuffer;
struct PLine PowerLine;
...
{    
    ...
    // the irout state machine needs to have its clock divider adjusted to generate the correct frequency for the IR LED, 
    // this is done in the program init function
    float clk_div = (float)clock_get_hz(clk_sys) / (float)IROUT_CLOCK;
...
void __not_in_flash_func()debug_out(uint16_t pattern) {
    // send pulses to the P_DEBUG pin

    pio_sm_put(pio0_pio, debugout_sm, pattern);
}
...
// returns the offset in FRAM if it exists, or 0 if it does not exist
uint32_t __not_in_flash_func()exist_usermem(uint32_t address)
{
    uint16_t xmem_mods;
    uint16_t umem_mods;
    ...
    if ((xmem_mods & xmem1_plugged) && (address > 0x200) && (address < 0x2F0)) {
        // return the offset in FRAM for this user memory address, this is for the case that there is no XMEM module plugged in, so we use the FRAM for user memory
        return (FRAM_XMEM1_start + (address - 0x200) * 8) ;
    }
    ...
    // now check for the user memory modules
    // Address will be in the range 0x100 to 0x1FF, but not all addresses will be valid depending on the number of UMEM modules plugged in
    if ((address >= 0x100) && (address < (0x100 + umem_mods * 0x40))) {
        // return the offset in FRAM for this user memory address, this is for the case that there is no XMEM module plugged in, so we use the FRAM for user memory
        return (FRAM_QUADMEM_start + (address - 0x100) * 8) ;
    }

    return 0;   // address does not exist
}
...
void PowerMode_task()
{
    ...
    if (HP41_powermode != prev_mode)
    {
        ...
        t_end = get_absolute_time();                                // this is now

        PowerLine.cycle_number = cycle_counter;
        PowerLine.new_mode = HP41_powermode;
        PowerLine.prev_mode = prev_mode;
...
void __not_in_flash_func(core1_pio)()
{
    ...
    uint16_t wrom_bank = 0;

    uint16_t wptog_page = 0;  // for WPTOG instruction
        ...
        { 
            ...
            fi_data_compressed = 0;
                ...
                if (pio_sm_get_rx_fifo_level(pio0, fiin_sm) > 0) {
                    ...
                    fi_data_compressed |= ((fi_word >>  6) & 0x1) << 1;   // FI1 in bit 6
                    fi_data_compressed |= ((fi_word >> 10) & 0x1) << 2;   // FI2 in bit 10
                    fi_data_compressed |= ((fi_word >> 14) & 0x1) << 3;   // FI3 in bit 14
                    fi_data_compressed |= ((fi_word >> 18) & 0x1) << 4;   // FI4 in bit 18
                    fi_data_compressed |= ((fi_word >> 22) & 0x1) << 5;   // FI5 in bit 22
                    fi_data_compressed |= ((fi_word >> 26) & 0x1) << 6;   // FI6 in bit 26
                ...
                if (pio_sm_get_rx_fifo_level(pio0, fiin_sm) > 0){
                    ...
                    fi_data_compressed |= ((fi_word >> 18) & 0x1) << 10;   // FI10 in bit 10
                    fi_data_compressed |= ((fi_word >> 22) & 0x1) << 11;   // FI11 in bit 14
                ...
                // compress data here as well
                // fi_out1 is FI 0...7, fi_out2 is FI 8..13
                fi_word = fi_out1;
                fi_data_compressed = 0;
                fi_data_compressed |= (fi_word >>  2 & 0x01) <<  0;       // FI0 in bit 2
                fi_data_compressed |= (fi_word >>  6 & 0x01) <<  1;       // FI1 in bit 6
                fi_data_compressed |= (fi_word >> 10 & 0x01) <<  2;       // FI2 in bit 10
                fi_data_compressed |= (fi_word >> 14 & 0x01) <<  3;       // FI3 in bit 14
                fi_data_compressed |= (fi_word >> 18 & 0x01) <<  4;       // FI4 in bit 18
                fi_data_compressed |= (fi_word >> 22 & 0x01) <<  5;       // FI5 in bit 22
                fi_data_compressed |= (fi_word >> 26 & 0x01) <<  6;       // FI6 in bit 26
            ...
            if ((rx_inst == inst_RAMSLCT) && (ourselected > 0)) {
                ...
                fram_offset = exist_usermem(ourselected); 
            ...
            page_addr = rom_addr & 0x0FFF;      // address within the page, right aligned
            ...
            if ((enabled_bank > 1) && !TULIP_Pages.isEnabled(rom_pg, enabled_bank) && TULIP_Pages.isEnabled(rom_pg, 1)) {
                ...
                if (TULIP_Pages.isEnabled(rom_pg, enabled_bank)) {
                    ...
                    // check if the polling entries are addressed and if polling is disabled
                    if ( !((page_addr >= 0x0FF4) && (page_addr <= 0x0FFA) && (TULIP_Pages.getflags(rom_pg,enabled_bank) & PAGE_POLLING))) {
                        // polling entries, but polling is disabled, so return empty data
                        isa_out_data = TULIP_Pages.getword(rom_addr, enabled_bank);  // get the word from Bank 1
                    }              
                }
            }
            ...
            switch (rx_inst) {
                case inst_WROM :                // WROM instruction
                    ...
                    // only for debug on the DevBoard!
                    // pio_sm_exec(pio1_pio, irout_sm, pio_encode_set(pio_x, 0) | pio_encode_sideset(1, 0));
                    // pio_sm_exec(pio1_pio, irout_sm, pio_encode_jmp(irout_offset + hp41_pio_irout_offset_hilo_loop) | pio_encode_sideset(1, 0));
                    ...
                    // find out the bank enabled for the page, as this is needed for the write
                    wrom_page = (wrom_addr & 0xF000) >> 12;   
                    wrom_bank = active_bank[wrom_page];          // this is the currently enabled bank for the page
                        
                    // write the data to the Page
                    // TULIP_Pages.writeword(wrom_addr, wrom_bank, wrom_data);     

                    // check if the Page is write protected
                    if ((TULIP_Pages.Pages[wrom_page].m_banks[1].b_img_flags & BANK_WRITEABLE) == BANK_WRITEABLE) {
                        // page is writeable, so we can write the data
                        // write prtection is valid for ALL banks in a Page
                        TULIP_Pages.writeword(wrom_addr, wrom_bank, wrom_data);    // for testing only bank 1 supported
                    }
                    break;  // end of case for WROM instruction

                case inst_HEPWPTOG:               // WPTOG instruction, toggle write protect bit for the page, must wait for DATA[2..0] to become available
                    // TraceLine.xq_data1 = rx_data1;
                    // TraceLine.xq_instr = rx_inst;
                    rx_inst_t = rx_inst;
                    wptog_page = (rx_data1 & 0x00F);   // isolate the page number from C[0], bank is always 1

                    // toggle the write protect bit for the page

                    // only HEPAX RAM supports WPTOG
                    if (TULIP_Pages.isEnabled(wptog_page, 1) && (TULIP_Pages.Pages[wptog_page].m_banks[1].b_img_flags & PAGE_HEPRAM)) {
                        // page is enabled, so we can toggle the write protect bit
                        if ((TULIP_Pages.Pages[wptog_page].m_banks[1].b_img_flags & BANK_WRITEABLE) == BANK_WRITEABLE) {
                            // currently write enabled, so clear the bit to enable write protection
                            TULIP_Pages.Pages[wptog_page].m_banks[1].b_img_flags &= ~BANK_WRITEABLE;
                        }
                        else {
                            // currently write protected, so set the bit to disable write protection
                            TULIP_Pages.Pages[wptog_page].m_banks[1].b_img_flags |= BANK_WRITEABLE;
                        }
                    }
                    break;  // end of case for WPTOG instruction
                ...
                case inst_RAMSLCT:              // RAMSLCT, must wait for DATA[2..0] to become available
                    ...
                    if (fram_offset != 0) {
                        ...
                        // read the whole cache line of 16 registers (16*8 bytes = 128 bytes) into the cache
                        // this is currentluy just for checking the timing of the FRAM access
                        // umem_cache_slct = ramselected & 0xFFF0;         // aligned on 16-register boundary
                        // fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, fram_offset & 0xFFFFFFF0, (uint8_t*)&umem_cache, 128);   
                    }
                    else {
