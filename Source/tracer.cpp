...
// Total occupied size of the above array is 
//  2048 entries 
// pointer table 8192 bytes, 4 bytes per entry
// string literals 16511 bytes, total 24703 bytes
// if pooling enabled 19356 bytes
...
bool send_pending = false;              // flag to indicate that the traceline was not sent due to a full buffer
...
void TraceBuffer_init()
{
    ...
    if (PreTrigLength < 0) PreTrigLength = 0;
    if (PreTrigLength > 256) PreTrigLength = 256;
    ...
    // retrieve the filter settings from FRAM and set the filter variables
    TraceFilter.retrieve();
}

bool SetPreTrigBuffer(int pretrig)
{
    // change the pre-trigger buffer size
    if (pretrig < 0) return false;          // negative pre-trigger size is not allowed
...
void Trace_task()
{
    ...
    if (cdc_connected_now[ITF_TRACE]) {
        ...
        if (cdc_connected_changed[ITF_TRACE]) {
            ...
            cdc_connected_changed[ITF_TRACE] = false;     // reset the flag, we have processed the change in connection status
            TracePrintLen = 0;
    ...
    if (send_pending) {
        ...
        if (cdc_connected_now[ITF_TRACE]) {
            send_pending = !cdc_sendbuf_available(ITF_TRACE, TracePrint, TracePrintLen);
    ...
    if (!queue_is_empty(&TraceBuffer)) {
        ...
        // first check is this is a sample with time tag
        if (TraceSample.data2 & (1 << 29)) {
            // this is a time tag sample, we can print the time tag and skip the rest of the processing for this sample

            memcpy(&tt_elapsed, &TraceSample.HPILregs[0], sizeof(int64_t));   // time tag is in data1 and data2, right justified, bit 29 of data2 is 1 to indicate time tag

            t_secs_elapsed = (float)tt_elapsed / (1000 * 1000);

            TracePrintLen = 0;
            TracePrintLen += sprintf(TracePrint + TracePrintLen, "\n** Time elapsed: %07.5f secs - ", t_secs_elapsed);

            PW_mode = TraceSample.ramslct;
            switch (PW_mode) {
                case 0x00:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "unknown event\n\r");
                    break;
                case 0x01:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Deep sleep -> Light sleep\n\r");
                    break;
                case 0x02:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Light sleep -> Deep sleep\n\r");
                    break;
                case 0x03:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Deep sleep -> Running\n\r");
                    break;
                case 0x04:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Light sleep -> Running\n\r");
                    break;
                case 0x05:
                    calc_speed = cycle / t_secs_elapsed;
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Running -> Light sleep, total %d cycles = %d cycles/sec\n\r", cycle, calc_speed);
                    break;
                case 0x06:
                    calc_speed = cycle / t_secs_elapsed;
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Running -> Deep sleep, total %d cycles = %d cycles/sec\n\r", cycle, calc_speed);
                    break;
                case 0x10:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "External trigger\n\r");
                    break;
                case 0x11:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Wake-up by ISA trigger\n\r");
                    break;
                default:
                    TracePrintLen += sprintf(TracePrint + TracePrintLen, "Unknown code %02X\n\r", PW_mode);
                    break;
            }
        ...
        block = false;
        if (globsetting.get(tracer_filter_on)) {
            // filter is active, check if address is in the filtered ranges
            block = TraceFilter.apply(addr);
        } 
        ...
        if (!block) {
            ...
            */
            ...
            drive_FI = (TraceSample.fi & 0x8000) != 0;   // FI driven by TULIP if bit 15 of fi is 1
            ...
            if (cdc_connected(ITF_TRACE)) {
                ...
                // as a test. keep the CLI alive by sending a NULL character
                // cdc_sendbuf(ITF_CONSOLE, "\0", 1);
            }
