...
// XMEM memory settings
#define     xmem_pages          60          // 
#define     xfun_plugged        0x01        
#define     xmem1_plugged       0x02        
#define     xmem2_plugged       0x04        
// use bit pattern to indicate which pages are plugged
// bit 0 = XFUN memory plugged          0x040 - 0x0BF
// bit 1 - XMEM #1 plugged              0x200 - 0x2FF
// bit 2 - XMEM #2 plugged              0x300 - 0x3FF

// User Memory settings
#define     umem_pages          61          // number of UMEM pages (0, 1, 2, 3, 4)
...
#define     module_offset       95          // default module map offset in FRAM
...
class GSettings { 
    ...
    uint16_t gsettings[gsettings_lastitem + 1];         // array where items are stored
    ...
    int set_default() {
        ...
        gsettings[rommap]              = 0;        // default rom map filename, empty for now
        gsettings[module_offset]       = FRAM_ROMMAP_start;    // default module
        gsettings[tracer_offset]       = FRAM_TRACER_start;    // default tracer settings
        gsettings[usermem_offset]      = FRAM_STATUS_REGS_start;
        gsettings[xmem_offset]         = FRAM_XMEM0_start;         // default xmem settings
    }
    ...
    inline uint32_t get_xmem_offset() {
        return gsettings[xmem_offset];
    }

    inline uint32_t get_user_mem_offset() {
        return gsettings[usermem_offset];
    }

    inline uint32_t get_tracer_offset() {
        return gsettings[tracer_offset];
    }

    // retrieve a current setting
    // used inline here to enable quick access from core1 loop
    inline uint16_t get(int idx) {
        if (idx <= gsettings_lastitem) {
