...
void uif_status()
{
    ...
    // uint16_t adc_value = adc_read();              // read the raw ADC value
    ...
    // cli_printf("*      cpu temp :   %.02f C, %.02f F, ADC %d", tempC, tempC*9/5+32, adc_value);
    ...
    switch (HP41_powermode) { 
      case PowerMode_DeepSleep:
        cli_printf("*    Power Mode = OFF (HP41 not running or not connected)");
        break;
      case PowerMode_Running:
        cli_printf("*    Power Mode = RUN (HP41 running)");
        break;
      case PowerMode_LightSleep :
        cli_printf("*    Power Mode = LIGHT SLEEP (HP41 on but not running)");
        break;
      default:
        cli_printf("*    Power Mode = UNKNOWN, no calculator connected?");
    }

    if ((HP41_powermode == PowerMode_LightSleep) || (HP41_powermode == PowerMode_DeepSleep)) {
      int64_t us_elapsed = absolute_time_diff_us(PowerLine.t_start, PowerLine.t_end); 
      float secs_elapsed = (float)us_elapsed / (1000 * 1000);
      int calc_speed = PowerLine.cycle_number / secs_elapsed;
      cli_printf("*    Latest HP41 cycle frequency:  %d cycles/sec", calc_speed);
    } else {
      cli_printf("*    no data to report calculator speed");
    }
...
int compare_openfile(FIL* fp, uint32_t offs, bool comp_qrom)  
// if comp_qrom is true, then the file is a QROM file 
{
  ...
  if (comp_qrom) {
    // for QROM files, the header is stored in a different format in FLASH, so we need to read it first and convert it to the standard format
    fr_read(offs, (uint8_t *)&MetaHH, sizeof(ModuleMetaHeader_t));
    MetaH = &MetaHH;
  } else {
  ...
  while ((toread > 0) && result) {
    ...
    if (comp_qrom) {
      fr_read(offs + sizeof(ModuleMetaHeader_t) + addr, qr_buf, readsize);  // read the corresponding data from FLASH for comparison
      for ( i = 0; i < read; i++) {
        if (buf[i] != qr_buf[i])   {
...
int import_file(const char *fname, int option, bool imp_qrom)
{
  ...
  // following file types not yet supported, but maybe in the future
  ...
  if ((offs != NOTFOUND) || (offs_qr != NOTFOUND))  {
    ...
    if ((option == import_update) || (option == import_compare)) {
      ...
      if (result == COMPARE_DIFF_SIZE) {
        cli_printf("  file size in FLASH/QROM is different, cannot compare or update");
        f_close(&fil);
      ...
      cli_printf("  file already in FLASH/QROM, cannot import again");     
      f_close(&fil);
      return import_file_exists;
    }
  ...
  if (option == import_compare) {
    cli_printf("  file not found in FLASH/QROM, can be imported");
    f_close(&fil);
    return import_file_notinflash;
  }
  ...
  if (!imp_qrom) {
    ...
    // import a file in QROM
    // could also be an UPDATE of a QROM file
    if (offs_qr == NOTFOUND) {
      cli_printf("  importing a new file in QROM");
    ...
    // if this is an update the file size is already checked in the compare function
    // so we can just program the file in FRAM at offs_qr

    // program the open file in QROM in blocks of 4K
    ...
    strcpy(header.FileName, fname);
    header.FileSize = filesize;
    header.NextFile = offs_qr + filesize + sizeof(header);   // may be overwritten later

    ModuleMetaHeader_t old_header;
    uint8_t end_of_filesystem = 0;

    // read the current file header at offs_qr to check if this is a new file or a deleted file
    fr_read(offs_qr, (uint8_t *)&old_header, sizeof(ModuleMetaHeader_t));

    // check if this was the end of the filesystem
    if (old_header.FileType == FILETYPE_END) {
      // this was the end of the filesystem, so we can set the next file offset to the end of the filesystem
    ...
    // if it was the last file then we can set the next file offset to the end of the filesystem
      ...
      uint8_t next_filetype;
      fr_read(old_header.NextFile, &next_filetype, 1);  // read the file type of the next file to check if this is the end of the filesystem
      if (next_filetype == FILETYPE_END) {
        // this is the end of the filesystem, so we can set the next file offset to the end of the filesystem
        end_of_filesystem = FILETYPE_END;
      }
    }
    ...
    // finally program the file in FRAM in blocks of 4K
    UINT read;
    // in case this is an UPDATE, the filepointer must be reset
    f_lseek(&fil, 0);
    uint32_t addr = offs_qr + sizeof(header);
    while (true) {
      fr = f_read(&fil, buf, 0x1000, &read);
      if (FR_OK != fr) {
        cli_printf("  file read error: %s (%d)", FRESULT_str(fr), fr);
        f_close(&fil);
        return import_nofile;
      ...
      fr_write(addr, buf, read);  // write the data to FRAM at the current address
      addr += read;               // increase the address for the next block
    }

    // program end of file system marker if this is a new file added to the end of the file system
    if (end_of_filesystem == FILETYPE_END) {
...
void uif_import_all(const char *dir, int arg)
{
  ...
  while (fr == FR_OK && fno.fname[0]) { /* Repeat while an item is found */
    ...
    if (fno.fattrib & AM_DIR) {
      ...
      res = import_file(fname, arg, false);   // for now only import to FLASH, not QROM
      result[res]++;
      cli_printf("");
    }
...
// import functions foor importing a file into QROM/FRAM
// a3 is passed with the following values:
// import_no_arg   import [file] q
...
void import_qrom(const char *fname, int a3) 
{
  ...
  if (offs != NOTFOUND) {
    // file exists in FLASH, show error message
    cli_printf("  file found in FLASH at 0x%08X, cannot import a file in QROM with the same name as one in FLASH", offs);
    f_close(&fil);
    return;
  }

  // now check if the file exists in qrom/fram
  offs = fr_findfile(fname);
  if ((offs != NOTFOUND) && (a3 != import_no_arg)) {
    cli_printf("  file already found in QROM/FRAM at 0x%08X, cannot import, use compare or UPDATE", offs);
    f_close(&fil);
    return;
  }

  // the file is alreday open and the extension is .ROM or .MOD
  

  if (offs == NOTFOUND) {
    // file not found, this is a new file to be imported
    // always import and create the file regardless of CUPDATE or compare option

    // first find if there is enough space for the file
    offs = fr_lastfree(filesize);
    if (offs == NOTFOUND ) {
      cli_printf("  no free space in QROM/FRAM");
      f_close(&fil);
      return;
    }
...
#define EXPORT_HELP_TXT "export functions\r\n\
        [filename]                   export the named file to the uSD card\r\n\
                                     will be stored in current directory of the uSD card and with the same name\r\n\
                                     existing files will NOT be overwritten\r\n\
        [filename] [dir/filename]    export a single file to the uSD card and define the name/directory\r\n\
        [filename] [dir]             the file will be stored in the named directory on the uSD card\r\n\
...
void uif_export(int func, const char *fname, const char *dir)
{
  uint32_t offs;
  FIL fil;
  
  // find the file fname in FRAM/QROM 
  offs = fr_findfile(fname);
  if (offs == NOTFOUND) {
    cli_printf("  file not found in QROM/FRAM, cannot export");
    return;
  }

  // check if there is a directory specified for the export
  if (func == export_file) {
    // no directory specified, export to current directory of uSD card with the same name
    
    // check if the file already exists on the uSD card in the current working directory
    FRESULT fr;
    FILINFO fno;
    fr = f_stat(fname, &fno);
    if (fr == FR_OK) {
      cli_printf("  file already exists on uSD card in current directory, cannot export with the same name");
      return;
    }

    // create the new file on the uSD card in the current directory with the same name
    // and save the file from QROM/FRAM to the uSD card in the current directory with the same name
    fr = f_open(&fil, fname, FA_WRITE | FA_CREATE_NEW);
    if (fr != FR_OK) {
      cli_printf("  cannot create file on uSD card: %s (%d)", FRESULT_str(fr), fr);
      return;
    }
    
    // get the size of the file to be exported from the header in FRAM
    ModuleMetaHeader_t header;
    fr_read(offs, (uint8_t *)&header, sizeof(header));
    uint32_t filesize = header.FileSize;
    cli_printf("  exporting file %s from QROM/FRAM to current directory of uSD card, size %d bytes", fname, filesize);  

    uint32_t addr = offs + sizeof(header);
    uint8_t buf[32];

    // read filesize bytes inn chuinks of 32 bytes and write to the file on the uSD card
    uint32_t bytes_left = filesize;
    while (bytes_left > 0) {
      uint32_t bytes_to_read = (bytes_left > 32) ? 32 : bytes_left;
      fr_read(addr, buf, bytes_to_read);

      f_write(&fil, buf, bytes_to_read, NULL);
      addr += bytes_to_read;
      bytes_left -= bytes_to_read;
    }

    // close the file on the uSD card
    f_close(&fil);
    return;
  } 
  
  if (func == export_file_dir) {
    // directory specified, export to the named directory on the uSD card with the same name
    // or a new files name specified, export to the current directory of the uSD card with the new name
    // or a direcory with filename specified, export to the named directory on the uSD card with the new name

    // first check the name string to see if it contains a directory and filename or just a directory
    char export_dir[FF_LFN_BUF] = {0};
    char export_fname[FF_LFN_BUF] = {0};
    ...
    fr = f_stat(dir, &fno);
    if (fr == FR_OK) {
      cli_printf("  directory/file already exists, cannot export with the same name");
      return;
    }

    // now create the file and save the file from QROM/FRAM to the uSD card in the current directory with the same name
    fr = f_open(&fil, dir, FA_WRITE | FA_CREATE_NEW);
    if (fr != FR_OK) {
      cli_printf("  cannot create file on uSD card: %s (%d)", FRESULT_str(fr), fr);
      return;
    }
    ...
    fr_read(offs, (uint8_t *)&header, sizeof(header));
    uint32_t filesize = header.FileSize;
    cli_printf("  exporting file %s to %s, size %d bytes", fname, dir, filesize);  
...
void uif_cd(int function, const char *dir)
{
  char cwdbuf[FF_LFN_BUF] = {0};
  FRESULT fr; /* Return value */
  char const *p_dir;

  if (function == cd_status) {
    // no directory specified, show the current working directory
    sd_getcwd(cwdbuf, FF_LFN_BUF);  // get the current working directory from the SD card
    cli_printf("  current working directory: %s", cwdbuf);
    return;
  }

  if (function == cd_root) {
    // change the working directory to the root directory
    fr = sd_chdir("/");  // change the working directory to the root directory on the SD card
    if (FR_OK != fr) {
      cli_printf("  cannot get current working directory: %s (%d)", FRESULT_str(fr), fr);
      return;
    }

    cli_printf("  current working directory is now: /");
    return;
  }

  // add a '/' before ythe directory name if not already present to ensure that we are changing to a directory in the root of the SD card
  char dirbuf[FF_LFN_BUF] = {0};
  if (dir[0] != '/') {
    snprintf(dirbuf, FF_LFN_BUF, "/%s", dir);
    p_dir = dirbuf;
  } else {
    p_dir = dir;
  }
  // change the directory to the specified directory
  cli_printf("  changing directory to: %s", p_dir);
  fr = sd_chdir(p_dir);
  if (FR_OK != fr) {
    cli_printf("  cannot change directory: %s (%d)", FRESULT_str(fr), fr);
    return;
  }
  sd_getcwd(cwdbuf, FF_LFN_BUF);  // get the current working directory from the SD card
  cli_printf("  current working directory is now: %s", cwdbuf);
}


// show the details of a ROM file given an offset QROM/FRAM
void ShowRomDetailsQ(uint32_t ROMoffset)
{
  ...
  {     
    char c = swap16(fr_read16(ROMoffset + (ROMName_offs - 1) * 2));
    int i = 0;
    while ( ((c & 0xFF) < 0x40) && (i < 16)) {
      // only allowed chars
      ROMname[i] = HPchar[c & 0x3F];
      i++;
      c = swap16(fr_read16(ROMoffset + (ROMName_offs - 1 - i) * 2));
    }
  ...
  cli_printf("    Rev        %c%c-%c%c", HPchar[swap16(fr_read16(ROMoffset + 2 * 0xFFE)) & 0x3F], 
                                         HPchar[swap16(fr_read16(ROMoffset + 2 * 0xFFD)) & 0x3F], 
                                         HPchar[swap16(fr_read16(ROMoffset + 2 * 0xFFC)) & 0x3F], 
                                         HPchar[swap16(fr_read16(ROMoffset + 2 * 0xFFB)) & 0x3F]);
  cli_printf("    Checksum   0x%03X", swap16(fr_read16(ROMoffset + 2 * 4095)));   
...
void ShowROMDumpQ(uint32_t ROMoffset)
{
  // given an offset in QROM/FRAM to the ROM contents, dumps the ROM file
  ...
  uint16_t romword;
  int m;
  uint16_t ROMLine[16];                     // buffer pointer for a line of the ROM image
  ...
  do {
    ...
    // read a line of 16 words from the ROM image in QROM/FRAM
    fr_read(ROMoffset + addr * 2, (uint8_t *)ROMLine, 32);   // read 16 words (32 bytes) from the ROM image in QROM/FRAM

    // print 16 bytes
    for (m = 0; m < 16; m++) {
      romword = swap16(ROMLine[m]);
...
// getfrombinQ is the same as getfrombin but for a byte array in QROM/FRAM, thus using fr_read to read the bytes
// inpout is the following
//   BINoffset - offset of the packed ROM file in FRAM/QROM
//   address - address of the word to be read in the ROM image (0-4095)

inline uint16_t getfrombinQ(uint32_t BINoffset, uint16_t address) {
  // uint8_t *bin = (uint8_t*)binarray;
  uint16_t res1, res2, result;
  uint16_t offset = (address * 5) / 4;            // offset in the packed ROM file of the first byte
  int shift1 = (address & 0x0003) * 2;
  ...
  // now read the two bytes from the BIN file in QROM/FRAM using fr_read
  uint8_t byte[2];
  fr_read(BINoffset + offset, byte, 2);   // read the two bytes from the BIN file in QROM/FRAM
  res1 = (byte[0] & mask1) >> shift1;    // get the first part of the word
  res2 = (byte[1] & mask2);
  res2 = res2 << shift2; // get the second part of the word
  result = res1 + res2;
  return result;
...
void ShowMODDetailsQ(uint16_t MODoffset)
{
  // given an offset in FLASH to the ROM contents, show the details of the ROM file
  ModuleFileHeader_t *myMODImage;             // pointer to the struct   
  ModuleFileHeader_t myMODHeader;             // copy of the MOD file Header
  ModulePageHeader_t myPageHeader;            // copy of the individual pages header

  int NumFunctions;

  uint32_t myImageoffset;                     // offset to the image data in QROM/FRAM, 
                                              // this is calculated from the page header

  // read the MOD file header from QROM/FRAM
  fr_read(MODoffset, (uint8_t *)&myMODHeader, sizeof(ModuleFileHeader_t));

  myMODImage = &myMODHeader;
  int ModNumPages = myMODImage->NumPages;
  cli_printf("    MOD Format %s", myMODImage->FileFormat);
  cli_printf("    MOD Title  %s", myMODImage->Title);
  cli_printf("    MOD Pages  %d", myMODImage->NumPages);
  ...
  while (i < ModNumPages) {
    ...
    // read the page header from QROM/FRAM
    fr_read(MODoffset + sizeof(ModuleFileHeader_t) + i * sizeof(ModuleHeader_t), (uint8_t *)&myPageHeader, sizeof(ModulePageHeader_t));
    ...
    cli_printf("    XROM      %3d", getfrombinQ(myImageoffset, 0));
    cli_printf("    # Funcs   %3d", NumFunctions);

    // get a pointer to the ROM name (first function)
    uint16_t ROMName_offs = ((getfrombinQ(myImageoffset, 3) & 0xFF)) + ((getfrombinQ(myImageoffset, 2) & 0xFF) * 256);
    if (getfrombinQ(myImageoffset, 2) > 0xFF) 
    {
      // usercode ROM, do not decode, just mention it
      cli_printf("    ROMName    <USERCODE>");
    }
    ...
    {
      char c = getfrombinQ(myImageoffset, ROMName_offs - 1);
      int i = 0;
      while ( ((c & 0xFF) < 0x40) && (i < 16)) {
        // only allowed chars
        ROMname[i] = HPchar[c & 0x3F];
        i++;
        c = getfrombinQ(myImageoffset, ROMName_offs - 1 - i);
      }
      // and add the last character of the ROM name plus a NULL
      ROMname[i] = HPchar[c & 0x3F];
      ROMname[i + 1] = 0;
    ...
    // now the ROM Revision and checksum
    cli_printf("    Rev        %c%c-%c%c", HPchar[getfrombinQ(myImageoffset, 0xFFE) & 0x3F], 
                                           HPchar[getfrombinQ(myImageoffset, 0xFFD) & 0x3F], 
                                           HPchar[getfrombinQ(myImageoffset, 0xFFC) & 0x3F], 
                                           HPchar[getfrombinQ(myImageoffset, 0xFFB) & 0x3F]);
    cli_printf("    Checksum   0x%03X", getfrombinQ(myImageoffset, 0xFFF));  
    i++;
  }
}

void ShowMODDumpQ(uint32_t MODoffset)
{
  // given an offset in FLASH to the ROM contents, dumps the first page of the MOD file
  // MODoffset is the offset in QROM/FRAM to the MOD file header

  ModuleFileHeader_t *myMODImage;               // buffer pointer to the MOD file in FLASH
  ModuleHeader_t *myPageHeader;                 // pointer to the individual pages header
  ModuleHeader_t myPageHeaderCopy;              // copy of the individual pages header + content
  uint8_t *myImage;                             // pointer to the image data
  int ModNumPages;
  ...
  // simply copy the complete page header + content of the first page to a local buffer in RAM
  // this is simpler than doing multiple fr_read for the header and then for the content
  fr_read(MODoffset + sizeof(ModuleFileHeader_t), (uint8_t *)&myPageHeaderCopy, sizeof(ModuleHeader_t)); 
  ...
  while (i < ModNumPages) {
    ...
    do {
      ...
      // print 16 bytes
      for (m = 0; m < 16; m++) {
        romword = getfrombin(myImage, addr + m);
        ShowPrintLen += sprintf(ShowPrint + ShowPrintLen, "%04X ", romword);
      }
...
void uif_rtc(int i, const char *args)    // RTC test functions
{
  ...
  switch (i) {
    ...
    case 3:     // RTC get
      ...
      cli_printf("  Reg[0] %02X ", buf[0]);
      cli_printf("  Reg[1] %02X ", buf[1]);
      cli_printf("  Reg[2] %02X ", buf[2]);
      cli_printf("  Reg[3] %02X ", buf[3]);
      ...
      if ((buf[0] & 0x20) == 0x00) {
        // bit 5 is the ST status bit, if 0 then clock is running
        cli_printf("  Clock is running");
      } else {
        cli_printf("  Clock is NOT running");
      }
...
// options for listing files in FRAM:
// #define list_no_arg         0   // filename only
// #define list_dump           1    
// #define list_ext          100 applicable here
// #define list_all            8 applicable here
// #define list_q              2        
// #define list_f              4 

void fr_list_files(int option) {
  ...
  // "...............................  ..  ........  ........  ........"
  cli_printf("  filename (FRAM/QROM)             type      size  address     next file");
  cli_printf("  -------------------------------  ----  --------  ----------  ----------");
  ...
  //  cli_printf("  -------------------------------  ----  --------  ----------  ----------");
      cli_printf("  ** Total files found (FRAM/QROM) %d", filecounter);
      cli_printf("  ** END OF FILE SYSTEM **         0x%08X", offs);
      cli_printf("  ** UNUSED SPACE UNTIL **         0x%08X - appr %d Kbytes free", end, free/1024);
      cli_printf("  **                               room for %d MOD or %d ROM files", free/5917, free/8192);
      return;
    }
    ...
    if ((option & list_all) == list_all && ((MetaH.FileType == FILETYPE_DELETED) || (MetaH.FileType == FILETYPE_DUMMY ))) {
      ...
      // cli_printf("  TULIP4041 FRAM HEADER file found, version: %s", versionstring);
    ...
    // TODO: extended listing is implemented later
    /*
    if (i == 2) {
...
#define LIST_HELP_TXT "list functions\r\n\
        ...
        <f/q> <e> <a>     f   - list only files in FLASH\r\n\
                          ...
                          e   - extended listing with more details per file for all files\r\n\
...
void uif_list(int option, const char *fname)
{
  ...
  bool file_in_fram = false;          // flag to indicate if the file is in FRAM
  
  ModuleMetaHeader_t FrMetaH;         // struct for the file header if the file is in FRAM, as we cannot map it to a struct like in FLASH
  ModuleFileHeader_t FrModH;          // struct for the MOD header if the file is in FRAM, as we cannot map it to a struct like in FLASH
  ModuleHeader_t FrPageH;             // struct for the page header if the file is in FRAM, as we cannot map it to a struct like in FLASH


  // check if the filesystem is initialized
  if (!ff_isinited()) {
    cli_printf("  FLASH File system not initialized, please check and run INIT");
    return;
  }
  if (!fr_isinited()) {
    cli_printf("  FRAM File system not initialized, please check and run INIT");
    return;
  }
  ...
  if (fname != NULL) {
    ...
    if (MetaH->FileType == FILETYPE_ROM) {
      ...
      if (file_in_fram) {
        // file is in FRAM, so we need to read the ROM details from FRAM
        cli_printf("  ROM file is in QROM/FRAM, reading details from FRAM");
        ShowRomDetailsQ(offs + sizeof(ModuleMetaHeader_t));

        if (option == list_dump) {
          // also show the ROM dump
          ShowROMDumpQ(offs + sizeof(ModuleMetaHeader_t));
        }
      } else {
        // file is in FLASH, we can read the ROM details directly from FLASH
        cli_printf("  ROM file is in FLASH, reading details from FLASH"); 
        ShowROMDetails((uint16_t*)(FF_SYSTEM_BASE + offs + sizeof(ModuleMetaHeader_t)));
        if (option == list_dump) {
            // also show the ROM dump
     ...
     return;
      ...
      if (file_in_fram) {
        // file is in FRAM, so we need to read the MOD details from FRAM
        ShowMODDetailsQ(offs + sizeof(ModuleMetaHeader_t));
        if (option == list_dump) {
          // also show the MOD dump
          ShowMODDumpQ(offs + sizeof(ModuleMetaHeader_t));
        }
      } else {
        // file is in FLASH, we can read the MOD details directly from FLASH
        cli_printf("  MOD file is in FLASH, reading details from FLASH");
      ...
      cli_printf("  details for this file type not supported");
      return;
    }
  }
  ...
  */
  ...
  // list the files in flash if required
  if ((option & list_f) == list_f) {
  
    // layout for listing
    //  filename                        type   size    address     next
    // "...............................  ..  ........  ........  ........"
    cli_printf("  filename (FLASH)                 type      size  address     next file");
    cli_printf("  -------------------------------  ----  --------  ----------  ----------");
    ...
    while (offs < end) {
      ...
      if (MetaH->FileType == FILETYPE_FFFF) {
        ...
        uint32_t free = end - offs;
    //  cli_printf("  -------------------------------  ----  --------  ----------  ----------");
        cli_printf("  ** Total files found (FLASH)     %d", filecounter);
        cli_printf("  ** END OF FLASH FILE SYSTEM      0x%08X", offs);
        cli_printf("  ** UNUSED SPACE UNTIL            0x%08X - appr %d Kbytes free", end, free/1024);
        cli_printf("  **                               room for appr %d MOD or %d ROM files", free/5917, free/8192);
        break;
      }

      // build the string for showing the listing
      if ((option & list_all) == list_all && ((MetaH->FileType == FILETYPE_DELETED) || (MetaH->FileType == FILETYPE_DUMMY ))) {
        // include erased files in the listing, so we show all files until the end of the flash  
        sprintf(line, "  %-31s  0x%02X  %8d  0x%08X  0x%08X", 
                MetaH->FileName, MetaH->FileType, MetaH->FileSize, offs, MetaH->NextFile);
        cli_printfn(line);
        filecounter++;  // count the number of files found
      ...
      if ((option & list_ext) == list_ext) {
        ...
        if ((MetaH->FileType == FILETYPE_MOD1) || (MetaH->FileType == FILETYPE_MOD2)) {
          ...
          // all other filetypes, just show the info
          // to be added later
      }
  ...
  // now list the files in fram, if required
  // TODO: FRAM listing does not yet support the extended listing with more details, this will be added later
  if ((option & list_q) == list_q) {
    fr_list_files(option);
  }
...
int LoadMOD(uint32_t offs, bool isFRAM)
{
  ...
  ModuleMetaHeader_t MetaFR;          // struct for the file header if the file is in FRAM, as we cannot map it to a struct like in FLASH

  ModuleFileHeader_t *pMFH;       // pointer to the Module structure
  ModuleFileHeader_t MFHFR;       // struct for the MOD header if the file is in FRAM, as we cannot map it to a struct like in FLASH

  ModuleHeader_t *pMFP;                 // pointer to the page header in the MOD file 
  ModuleHeader_t MFPFR;                 // struct for the page header if the file is in FRAM, as we cannot map it to a struct like in FLASH
  ...
  if (isFRAM) {
    // file is in FRAM, read the header from FRAM
    fr_read(offs, (uint8_t *)&MetaFR, sizeof(ModuleMetaHeader_t));
    MetaH = &MetaFR; // point to the header read from FRAM

    // now read the module file header from FRAM
    fr_read(offs + sizeof(ModuleMetaHeader_t), (uint8_t *)&MFHFR, sizeof(ModuleFileHeader_t));
    pMFH = &MFHFR; // point to the module file header read from FRAM
  } else {
    // file is in FLASH, map the header to the struct
    MetaH = (ModuleMetaHeader_t*)(FF_SYSTEM_BASE + offs);       
...
// first check if the hardware is supported by the TULIP firmware
  ...
  for (int pass = 1; pass <= 3; pass++) {
    for (int pageIndex = 0; pageIndex < pMFH->NumPages; pageIndex++) {
      ...
      // read the page header if it is in FRAM, otherwise we can read it directly from FLASH
      if (isFRAM) {
        // page header is in FRAM, so we need to read it from FRAM
        fr_read(offs + sizeof(ModuleMetaHeader_t) + sizeof(ModuleFileHeader_t) + pageIndex * sizeof(ModuleHeader_t), 
                (uint8_t *)&MFPFR, sizeof(ModuleHeader_t));
        pMFP = &MFPFR; // point to the page header read from FRAM
      } else {
        // page header is in FLASH, we can read it directly from FLASH  
      ...
      if (fLoad) {   
        ...
        Bank_Flags = BANK_ACTIVE | BANK_ENABLED;  // always active, enabled and in FLASH or FRAM

        if (pMFP->RAM) {
          Bank_Flags |= BANK_QROM + BANK_WRITEABLE;    // ignored when in FLASH, not writeable anyway
          if (pMFH->Hardware == HARDWARE_HEPAX) Bank_Flags |= BANK_HEPRAM;  // mark as HEPAX RAM
        }         

        if (pMFP->WriteProtect) Bank_Flags &= ~BANK_WRITEABLE;              // ignored when in FLASH, not writeable anyway

        if (!isFRAM)            Bank_Flags |= BANK_FLASH;                   // file is in FLASH, so we need to set the flag for FLASH
        ...
        // and plug the image
        if (isFRAM) {
          // file is in FRAM, so we need to set the pointer to the image in FRAM
          TULIP_Pages.Pages[page].m_banks[pMFP->Bank].b_img_data = (uint16_t *)(offs + sizeof(ModuleMetaHeader_t) + 
                                                                                       sizeof(ModuleFileHeader_t) + 
                                                                                       (pageIndex) * sizeof(ModuleHeader_t) +
                                                                                       sizeof(ModulePageHeader_t));  
                                                                                       // hard address of the image in FRAM
          TULIP_Pages.Pages[page].m_banks[pMFP->Bank].b_img_rom  = offs; // offset to the MOD file in FRAM
          TULIP_Pages.Pages[page].m_banks[pMFP->Bank].b_img_file = offs; // offset to the MOD file in FRAM
        } else {
          // file is in FLASH, so we can set the pointer to the image in FLASH directly
...
void uif_plug(int func, int Page, int Bank, const char *fname)          // plug the selected ROM 
{
  ...
  bool FileInFRAM = false; // flag to indicate if the file is in FRAM or FLASH
  ...
  switch (func)
  {
    ...
    case plug_file_X: // plug the give filename in the given Page
            ...
            if (MetaH->FileType == FILETYPE_ROM) {
              ...
              // and prepare to print more details from the MOD/ROM content below
              if (FileInFRAM) {
                // file is in FRAM, get some details from there
                ShowRomDetailsQ(offs + sizeof(ModuleMetaHeader_t)); // show details from the ROM header in FRAM
              } else {
                // file is in FLASH, get a pointer to the ROM image in FLASH
              ...
              */ 
              ...
              if (FileInFRAM) {
                // file is in FRAM, so use the FRAM offset for plugging
                // when in FRAM it is made writeable by default
                rom_flags = BANK_ACTIVE | BANK_ROM | BANK_ENABLED | BANK_WRITEABLE;   // prepare the flags for the ROM in FRAM
              } else {
                // file is in FLASH, so use the FLASH offset for plugging
            ...
            if (MetaH->FileType == FILETYPE_MOD1 || MetaH->FileType == FILETYPE_MOD2) {
              cli_printf("  only ROM files supported in a specified Page, use plug without a Page number for MOD files");
              return;
            }
            break;
    ...
    case plug_file_T: // autoplug the given filename in the first free Page, Test option
            ...
            if (MetaH->FileType == FILETYPE_ROM) {
              ...
              if (FileInFRAM) {
                // file is in FRAM, get some details from there
                ShowRomDetailsQ(offs + sizeof(ModuleMetaHeader_t)); // show details from the ROM header in FRAM
                ROMImage_offs = offs + sizeof(ModuleMetaHeader_t); // get the offset to the ROM image in FRAM
                cli_printf("    ROM Image offset in FRAM at 0x%08X, now AutoPlugging", offs);
                rom_flags = BANK_ACTIVE | BANK_ROM | BANK_ENABLED | BANK_WRITEABLE; // prepare the flags for the ROM
              } else {
                // file is in FLASH, get a pointer to the ROM image in FLASH
                myROMImage = (uint16_t*)(FF_SYSTEM_BASE + offs + sizeof(ModuleMetaHeader_t));
                ShowROMDetails(myROMImage);
                ROMImage_offs = (uint32_t)myROMImage - (uint32_t)FF_SYSTEM_BASE; // get the offset to the ROM image in FLASH
                cli_printf("    ROM Image offset in FLASH at 0x%08X, now AutoPlugging", offs);
                rom_flags = BANK_ACTIVE | BANK_FLASH | BANK_ROM | BANK_ENABLED; // prepare the flags for the ROM
              }
              ...
              */
              ...
              int res = LoadMOD(offs, FileInFRAM);
              if (res >= LOADMOD_FAIL) {
                // show the error
                switch (res) {
                ...
                switch (res) {
                  ...
                  case HARDWARE_HEPAX:
                    cli_printf("  HEPAX Hardware detected, module plugged, only WPTOG emulated");
                    break;
...
void uif_qrom(int func, int p, int b)
{
  uint16_t flags;
  char status_str[20];
  char wp_status[8];
  char polling_status[8];
  char FileNm [33];
  char typestr[8]; // buffer for the type string
  ...
  switch (func) {
    case qrom_status:
      if (p == 0) {
        // show the status of all files plugged in FRAM 
        // the status can be the following:
        // - QROM
        // - ROM in FRAM, not QROM
        // - HEPRAM
        // - read enabled or disabled
        // - Write protected or write enabled
        // - Polling enabled or disabled

        cli_printf("  QROM/FRAM Pages status:");
        cli_printf("  Page - Bank - XROM - Flags  - Type   - Enabled - Write   - Polling  - File  ");
        cli_printf("  ----   ----   ----   ------   ------   -------   -------   --------   ---------------------");                                               
        //                                         HEPRAM    yes      protect    enable
        //                                         QROM               enable     disable
        //                                         ROM
        //                                         FLASH     no
        ...
        for (int pg = 0; pg <= 16; pg++) {
          ...
          //go through all Pages and Banks 0..F, first read the flags of the page
            flags = TULIP_Pages.getflags(pg, bk);

            // We list pages in FRAM and Pages in FLASH that are disabled or with polling disable
            if ((flags & PAGE_ACTIVE) == 0) {
              continue;    // not an active Page, skip this
            }
          
            // check if the page is a QROM page, 
            // also disabled FLASH Pages or disabled polling are listed

            if (flags & PAGE_FLASH) {
              // check FLASH Pages
              if (((flags & PAGE_ENABLED) == PAGE_ENABLED) && (flags & PAGE_POLLING) == 0) {
                continue;
              }
            }
          
            // find out type of the ROM, this is based on the flags, but we want to show it as a string
            if (flags & PAGE_FLASH) {
            // this is a FLASH ROM
              snprintf(typestr, sizeof(typestr), "%s", "FLASH ");
            } else if (flags & PAGE_HEPRAM) {
              // this is HEPRAM
              snprintf(typestr, sizeof(typestr), "%s", "HEPRAM");
            } else if (flags & PAGE_QROM) {
              // this is a QROM
              snprintf(typestr, sizeof(typestr), "%s", "QROM  ");
            } else {
              // this is a ROM plugged in FRAM
              snprintf(typestr, sizeof(typestr), "%s", "FRAM  ");
            }

            // get the enabled status
            if (flags & PAGE_ENABLED) {
              // Page is enabled
              snprintf(status_str, sizeof(status_str), "%s", "YES");
            } else {
              snprintf(status_str, sizeof(status_str), "%s", "NO ");
            }

            // get the polling status
            if (flags & PAGE_POLLING) {
              // Polling is enabled
              snprintf(polling_status, sizeof(polling_status), "%s", "DISABLED");
            } else {
              snprintf(polling_status, sizeof(polling_status), "%s", "ENABLED ");
            }
                 
            // get the Write Protect status
            if (flags & PAGE_WRITEABLE) {
              // Page is writeable
              snprintf(wp_status, sizeof(wp_status), "%s", "ENABLED");
            } else {
              snprintf(wp_status, sizeof(wp_status), "%s", "PROTECT");
            }
       ...
       //                                            FLASH     no
            if (bk ==1 ) { 
              cli_printf("  %3X      1    %4d   0x%04X   %s     %s     %s   %s    %s", pg, 
              ...
              // skip Page number for readability
              cli_printf("           %d    %4d   0x%04X   %s     %s     n/a   n/a    %s", bk,
                                                                                        TULIP_Pages.getword((pg << 12) + (bk-1)*0x400, 1), 
                                                                                        flags,
                                                                                        typestr,
                                                                                        status_str, 
                                                                                        // wp_status,
        ...
        // p != 0, so show the detailed status of the QROM in Page p (hex)
        if (p < 4 || p > 15) {
          cli_printf("  Invalid Page number, must be between 4 and F");
          return;
        } 
      }
      break;
    ...
    case qrom_enable:
      // toggle the enable of the QROM in Page p (hex)
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }
      flags = TULIP_Pages.getflags(p, 1);
      if (flags & PAGE_ACTIVE) {
        // toggle the enable flag
        if (flags & PAGE_ENABLED) {
          // currently enabled, so disable it
          flags &= ~PAGE_ENABLED;
          cli_printf("  QROM in Page %X disabled", p);
        } else {
          // currently disabled, so enable it
          flags |= PAGE_ENABLED;
          cli_printf("  QROM in Page %X enabled", p);
        }
        TULIP_Pages.setflags(p, 1, flags); // update the flags for the page
        TULIP_Pages.save(); // save the page settings in FRAM
      } else {
        cli_printf("  Page %X does not have a ROM plugged", p);
      }
      break;

    case qrom_qrom:
      // toggle the QROM status of the file in Page p (hex) and Bank b
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }
      flags = TULIP_Pages.getflags(p, b);

      // check if the Page/Bank is active
      if ((flags & PAGE_ACTIVE) == 0) {
        cli_printf("  Page %X Bank %d does not have a ROM plugged", p, b);
        return;
      }

      if (flags & PAGE_QROM) {
        // toggle the QROM flag
        flags &= ~PAGE_QROM; // remove the QROM flag to make it a normal ROM
        cli_printf("  Page %X Bank %d is now a normal ROM, not QROM", p, b  );
        TULIP_Pages.setflags(p, b, flags); // update the flags for the page
        TULIP_Pages.save(); // save the page settings in FRAM
      } else {
        // toggle to make it a QROM
        flags |= PAGE_QROM; // set the QROM flag to make it a QROM
        cli_printf("  Page %X Bank %d is now QROM", p, b);
        TULIP_Pages.setflags(p, b, flags); // update the flags for the page
        TULIP_Pages.save(); // save the page settings in FRAM
      }
      break;
    ...
    case qrom_wprot:
      // toggle the write protection of the QROM in Page p (hex)
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }

      // checkif the Page is active and in FRAM, as write protection only applies to QROMs in FRAM
      flags = TULIP_Pages.getflags(p, 1);

      if (((flags & PAGE_ACTIVE) == 0) && ((flags & PAGE_FLASH) == 0)) {
        cli_printf("  Page %X does not have a ROM plugged", p);
        return;
      }

      if (flags & PAGE_WRITEABLE) {
        // currently writeable, so protect it
        flags &= ~PAGE_WRITEABLE;
        cli_printf("  QROM in Page %X (all Banks) is now write protected", p);
      } else {
        // currently protected, so make it writeable
        flags |= PAGE_WRITEABLE;
        cli_printf("  QROM in Page %X is now write enabled", p);
      }
      TULIP_Pages.setflags(p, 1, flags); // update the flags for the page
      TULIP_Pages.save(); // save the page settings in FRAM
    ...
    case qrom_hepram:
      // toggle the HEPAX RAM status of the QROM in Page p (hex)
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }
      flags = TULIP_Pages.getflags(p, 1);
      if (flags & PAGE_QROM) {
        // toggle the HEPAX RAM flag
        if (flags & PAGE_HEPRAM) {
          // currently has HEPAX RAM, so remove it
          flags &= ~PAGE_HEPRAM;
          cli_printf("  QROM in Page %X is now unmarked as HEPAX", p);
        } else {
          // currently does not have HEPAX RAM, so add it
          flags |= PAGE_HEPRAM;
          cli_printf("  QROM in Page %X is now marked as HEPRAM", p);
        }
        TULIP_Pages.setflags(p, 1, flags); // update the flags for the page
        TULIP_Pages.save(); // save the page settings in FRAM
      } else {
        cli_printf("  Page %X does not have a QROM plugged", p);
      }
      break;

    case qrom_polling:
      // toggle the polling interrupt enable in Page p (hex)
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }
      ...
      if (flags & PAGE_POLLING) {
        // currently enabled, so disable it
        flags &= ~PAGE_POLLING;
        cli_printf("  Polling interrupt in Page %X disabled", p);
      } else {
        // currently disabled, so enable it
        flags |= PAGE_POLLING;
        cli_printf("  Polling interrupt in Page %X enabled", p);
      }
    ...
    case qrom_clear:
      // clear the QROM in Page p and Bank b (1..4)
      if (p < 4 || p > 15) {
        cli_printf("  Invalid Page number, must be between 4 and F");
        return;
      }
      ...
      if (b < 1 || b > 4) {
        cli_printf("  Invalid Bank number, must be between 1 and 4");
        return;
      }

      if (TULIP_Pages.getflags(p, 1) & PAGE_WRITEABLE) {
        // page is write enabled, so we can clear the RAM
        uint32_t addr = (uint32_t)TULIP_Pages.Pages[p].m_banks[b].b_img_data; // get the base address of the page in FLASH or FRAM
        uint8_t zero_block[16] = {0};

        // check if image is a ROM or MOD1 file
        if (TULIP_Pages.getflags(p, 1) & PAGE_ROM) {
          // this is a ROM image, so it has a size of 0x2000, so we need to clear the whole image
          for (int w = 0; w < ROM_IMAGE_SIZE * 2; w += 16) {
            fr_write(addr + w, zero_block, 16); // write 16 bytes of 0 to the page
          }
        } else {
          // this is a compressed MOD1 image
          // the rest of the page is not used for RAM and can be left as it is
          for (int w = 0; w < MOD_IMAGE_SIZE; w += 16) {
            fr_write(addr + w, zero_block, 16); // write 16 bytes of 0 to the page
          }
        }

        cli_printf("  HEPRAM Page %X cleared, all words set to 0", p);
      } else {
        cli_printf("  HEPRAM Page %X is write protected, cannot clear", p);
      }
    ...
    default:
      cli_printf("  Invalid function for QROM, please check the help");
      break;
  }
}


#define max_xrom 25       // XROM number of the Extended Functions Module

bool xrom_used(int xrom) {
  // check if the XROM is used in any of the Pages
  for (int pg = 4; pg <= 16; pg++) {
    int value = TULIP_Pages.getXROM(pg, 1);
    if (value == xrom) {
      // XROM is used in this Page, so return true
      return true;
    }
  }
  return false; // XROM is not used in any Page
}

// #define hepram_status   1
// #define hepram_init     2

void uif_hepram(int func, int p)
{
  uint16_t flags;
  char status_str[20];
  char wp_status[8];
  ...
  switch (func) {
    case hepram_status:
      ...
      cli_printf("  HEPRAM/QROM Pages plugged:");

      // cli_printf("  Page - XROM - Flags - Type - Chain - Write - Prev  - Next  - File  ");
      cli_printf("  Page - XROM - Flags - Type                - Write   - Prev  - Next  - File  ");
      cli_printf("  ----   ----   -----   -------------------   -------   xFE7    xFE8    ---------------------");                                               

      for (int pg = 0; pg < 16; pg++) {
        //go through all Pages 0..15, first read the flasg of the page
        flags = TULIP_Pages.getflags(pg, 1);
        
        // check if the page is marked as HEPRAM or QROM
        if ((flags & PAGE_HEPRAM) || (flags & PAGE_QROM)) {
          // found a QROM or HEPRAM page, so show the details, all other pages are ignored
          
          // build the status string based on the flags
          // we want to list the following status values:
          // - QROM but no HEPRAM           NO HEPRAM
          // - HEPRAM but not initialized   NO INIT
          // - HEPRAM initialized           OK INIT

          if (!(flags & PAGE_HEPRAM)) {
            // QROM but no HEPRAM
            snprintf(status_str, sizeof(status_str), "%s", "QROM, NO HEPRAM    ");
          } else {

            // check if the page is initialized
            // according to the 1988 FileSys presentation by Steen Petersen this is the case if:
            //  words at x000, x001 and xFFF are all empty 
            if (TULIP_Pages.getword((pg << 12) + 0xFF3, 1) == 0x100) {
              // page is HEPRAM page, so it is initialized but has no HEPAX support
              snprintf(status_str, sizeof(status_str), "%s", "HEPRAM, NOT CHAINED");
            }
            if (TULIP_Pages.getword((pg << 12) + 0xFF3, 1) == 0x200) {
              // page is HEPRAM page, so it is initialized but has no HEPAX support
              snprintf(status_str, sizeof(status_str), "%s", "HEPRAM, CHAINED    ");
            }
            if ((TULIP_Pages.getword((pg << 12) + 0xFF3, 1) != 0x200) && ((TULIP_Pages.getword((pg << 12) + 0xFF3, 1) != 0x100))) {
              // page is HEPRAM page, but it is not initialized and not reserved for a module, so it should not be used
              snprintf(status_str, sizeof(status_str), "%s", "HEPRAM, RESERVED   ");
            }  
          }
          ...
          // TULIP_Pages.getFileName(pg, 1, FileNm);           // get the file name of the reserved page

       // cli_printf("  Page - XROM - Flags - Status               - Write   - Prev - Next - File  ");
       // cli_printf("  ----   ----   -----   --------------------   -------   xFE7   xFE8   ---------------------");

          if (flags & PAGE_HEPRAM) {
            cli_printf("  %3X    %3d    %04X    %s   %s   0x%03X   0x%03X   %s", pg, 
                                                                                 TULIP_Pages.getword(pg << 12, 1), 
                                                                                 flags,
                                                                                 status_str, wp_status,
                                                                                 TULIP_Pages.getword((pg << 12) + 0xFE7, 1),
                                                                                 TULIP_Pages.getword((pg << 12) + 0xFE8, 1),
    ...
    case hepram_initall:
      ...
      for (int pg = 0; pg < 16; pg++) {
        ...
        thisPage = pg;
        flags = TULIP_Pages.getflags(pg, 1);
        uint32_t Pageaddr = pg << 12;

        if (flags & PAGE_HEPRAM) {
          // this is HEPRAM, all other pages are ignored
          // initialize the page by writing 0 to all words, except for the word at x
          ...
          if ((flags & PAGE_WRITEABLE) == 0) {
            cli_printf("  HEPRAM Page %X is write protected, skip initialization and will not be in the HEPRAM chain", pg);
            continue; // skip the initialization of this page and go to the next one
          }
          
          cli_printf("  Initializing HEPRAM Page %X", pg);

          /*
          pagesize = MOD_IMAGE_SIZE;
          if (flags & PAGE_ROM) pagesize = ROM_IMAGE_SIZE * 2;

          // clear in blocks 0f 16 bytes, as this is faster than clearing word by word
          uint8_t zero_block[16] = {0};
          uint32_t addr = (uint32_t)TULIP_Pages.Pages[pg].m_banks[1].b_img_data; // get the base address of the page in FLASH or FRAM
          for (int w = 0; w < pagesize; w += 16) {
            fr_write(addr + w, zero_block, 16); // write 16 bytes of 0 to the page
          }

          cli_printf("  HEPRAM Page %X initialized, all words set to 0", pg);
          */

          // find out if the intended XROM numbers is already in use
          // in any case we cannot use the assigned numers for TIME, and XFunctions in the HP41CX)
          while (xrom_used(XROM) && (XROM < max_xrom)) {
            // cli_printf("  Intended XROM number 0x%02X is already in use, trying next one", XROM);
            XROM++; 
          }
          
          if (XROM >= max_xrom) {
            cli_printf("  No free XROM numbers available, cannot initialize HEPRAM Page %X", pg);
            cli_printf("  Abandoning HEPRAM Initialization");
            return;     // skip the initialization
          }

          // program the XROM number only if it was zero before
          if (TULIP_Pages.getword(Pageaddr, 1) == 0) {
            TULIP_Pages.writeword(Pageaddr, 1, XROM); // write the XROM number to the page header
            cli_printf("  HEPRAM Page %X programmed with XROM number 0x%02X", pg, XROM);
            XROM++; // increment the XROM number for the next HEPRAM page, if any
          }

          
          // build the rest of the HEPRAM chain information
          //    xFE7    block address of previous HEPRAM page in the chain, or 0 if this is the first page
          //    xFE8    block address of next HEPRAM page in the chain, or 0 if this is the last page
          //    xFE9    initialized to 0x091    start of first multi-block file with 0xFEA
          //    xFEA    initialized to 0x000
          //    xFEB    initialized to 0x000    start of first active file
          //    xFEC    initialized to 0x000
          //    xFED    initialized to 0x090    address of first word of filesystem in this block
          //    xFEE    initialized to 0x000
          //    xFEF    initialized to 0x091    address of first unused word in this block
          //    xFF0    initialized to 0x000
          //    xFF1    initialized to 0x0E5    last word inthis block of file system
          //    xFF2    initialized to 0x00F
          //    xFF3    initialized to 0x200    to indicate an active HEPRAM chain
          //                                    when 0x100 HEPRAM but not initialized
          //                                    anything else means reserved, not part of HEPRAM chain
          TULIP_Pages.writeword(Pageaddr + 0xFE7, 1, prevPage); // write the block address of the previous HEPRAM page in the chain
          TULIP_Pages.writeword(Pageaddr + 0xFE8, 1, nextPage); // write the block address of the next HEPRAM page in the chain

          // only overwerite the initialization values if they are zero, to avoid overwriting reserved pages
          if (TULIP_Pages.getword(Pageaddr + 0xFE9, 1) == 0) {
            TULIP_Pages.writeword(Pageaddr + 0xFE9, 1, 0x091   ); // write the value 0x091 to xFE9
          }

          TULIP_Pages.writeword(Pageaddr + 0xFED, 1, 0x090   ); // write the value 0x090 to xFED

          if (TULIP_Pages.getword(Pageaddr + 0xFEF, 1) == 0) {
            TULIP_Pages.writeword(Pageaddr + 0xFEF, 1, 0x091   ); // write the value 0x091 to xFEF
          }

          TULIP_Pages.writeword(Pageaddr + 0xFF1, 1, 0x0E5   ); // write the value 0x0E5 to xFF1
          TULIP_Pages.writeword(Pageaddr + 0xFF2, 1, 0x00F   ); // write the value 0x00F to xFF2

          TULIP_Pages.writeword(Pageaddr + 0xFF3, 1, 0x200   ); // write the value 0x200 to xFF3 to indicate an active HEPRAM page

          // Normally we do not know the address of the next HEPRAM Page
          // if the previous HEPRAM page is not zero, we need to enter the address of the current page there
          if (prevPage != 0) {
            TULIP_Pages.writeword((prevPage << 12) + 0xFE8, 1, thisPage); 
          }

          // update the previous page address for the next iteration
          prevPage = thisPage;
        }
      }
      TULIP_Pages.save(); // save the page settings in FRAM after initialization
      break;

    case hepram_clram:
      // clear the RAM in HEPRAM Page p
      // first check if the Page is a HEPRAM page and if it is write protected
      if (TULIP_Pages.getflags(p, 1) & PAGE_HEPRAM) {
        // this is a HEPRAM page, check if it is write protected
        if (TULIP_Pages.getflags(p, 1) & PAGE_WRITEABLE) {
          // page is write enabled, so we can clear the RAM
          uint32_t addr = (uint32_t)TULIP_Pages.Pages[p].m_banks[1].b_img_data; // get the base address of the page in FLASH or FRAM
          uint8_t zero_block[16] = {0};
          ...
          if (TULIP_Pages.getflags(p, 1) & PAGE_ROM) {
            ...
            for (int w = 0; w < ROM_IMAGE_SIZE * 2; w += 16) {
             ...
             // this is a normal MOD1 image, so it has a size of 0x1000, so we only need to clear the first 0x1000 bytes
             // the rest of the page is not used for RAM and can be left as it is
            for (int w = 0; w < MOD_IMAGE_SIZE; w += 16) {
              fr_write(addr + w, zero_block, 16); // write 16 bytes of 0 to the page
            }
        ...
        cli_printf("  Page %X is not a HEPRAM page, cannot clear", p);
      }
      break;

    case hepram_ramtog:
      // toggle the write protect status of HEPRAM Page p
      // first check if the Page is a HEPRAM page
      if (TULIP_Pages.getflags(p, 1) & PAGE_HEPRAM) {
        // this is a HEPRAM page, toggle the write protect status
        if (TULIP_Pages.getflags(p, 1) & PAGE_WRITEABLE) {
          // page is currently write enabled, so disable the write access
          TULIP_Pages.Pages[p].m_banks[1].b_img_flags &= ~BANK_WRITEABLE;

          cli_printf("  HEPRAM Page %X is now write protected", p);
        } else {
          // page is currently write protected, so enable the write access
          TULIP_Pages.Pages[p].m_banks[1].b_img_flags |= BANK_WRITEABLE; // set the writeable flag
          cli_printf("  HEPRAM Page %X is now write enabled", p);
        }
      } else {
        cli_printf("  Page %X is not a HEPRAM page, cannot toggle write protect status", p); 
      }
      TULIP_Pages.save(); // save the page settings in FRAM
      break;

    case hepram_release:
      // release HEPRAM Page p so it can be included in the HEPRAM chain, write 0x100 to xFF3
      // the chain must still be re-initialzed by the HEPAX moudle itself
      // first check if the Page is a HEPRAM Page
      if (TULIP_Pages.getflags(p, 1) & PAGE_HEPRAM) {
        // this is a HEPRAM page, so we can release it by writing 0x100 to xFF3
        TULIP_Pages.writeword((p << 12) + 0xFF3, 1, 0x100); // write the value 0x100 to xFF3 to indicate a reserved HEPRAM page
        cli_printf("  HEPRAM Page %X released, it is now part of the HEPRAM chain but not initialized", p);
      } else {
        cli_printf("  Page %X is not a HEPRAM page, cannot release", p);
      }
      break;

    case hepram_reserve:
      // reserve HEPRAM Page p for a specific purpose by writing a specific value to xFF3, so that it is not initialized and not part of the HEPRAM chain, but the content and the XROM number are kept, so that it can be reused as a reserved HEPRAM page
      if (TULIP_Pages.getflags(p, 1) & PAGE_HEPRAM) {
        // this is a HEPRAM page, so we can reserve it by writing a specific value to xFF3, for example 0x000 to indicate a reserved HEPRAM page
        TULIP_Pages.writeword((p << 12) + 0xFF3, 1, 0x000); // write the value 0x000 to xFF3 to indicate a reserved HEPRAM page
        cli_printf("  HEPRAM Page %X reserved, will not be part of the HEPRAM chain", p);
      } else {
        cli_printf("  Page %X is not a HEPRAM page, cannot reserve", p);
      }
      break;
...
void uif_cat(int p, int b)
{
  ...
  if (p == 0) {
    ...
    cli_printf("  Page - Bank - XROM -  Rev  - #Funcs - Type   - Flags - File  ");
    cli_printf("  ----   ----   ----   -----   ------   ------   ----   ---------------------");

 // cli_printf("  Page - Bank - XROM -  Rev  - #Funcs - Mem   - Write   - Flags  - Address    - File  ");
 // cli_printf("  ----   ----   ----   -----   ------   -----   -------   ------   ----------   ---------------------------");
   //                                                   EMBED   -    
   //                                                   FLASH   -    
   //                                                   QROM    enable    0x1234   0x12345678 - reserved : HP41 System ROM0
   //                                                   QROM    protect   
       ...
       // cli_printf("  ----   ----   ----   -----   ------   ------   ----   ---------------------");
        ...
        // check if the page is plugged and has a valid ROM image
          if (!TULIP_Pages.isPlugged(pg, bk)) {        
           ...
           // cli_printf("  ----   ----   ----   -----   ------   ------   ----   ---------------------");
            if (bk == 1) {
              cli_printf("  %3X    %3d    %4d   %s     %2d     %s   %04X    * %-25s  @ 0x%08X", pg, bk, TULIP_Pages.getword(addr + 0x00, bk), 
                                                                                    rev, 
              ...
              cli_printf("         %3d    %4d   %s     %2d     %s   %04X    * %-25s  @ 0x%08X", bk, TULIP_Pages.getword(addr + 0x00, bk), 
                                                                                    rev, 
                                                                                    TULIP_Pages.getword(addr + 0x01, bk),
                                                                                    typestr, 
                                                                                    TULIP_Pages.getflags(pg, bk),                                                                                    
           ...
           // cli_printf("  ----   ----   ----   -----   ------   -----   ---------------------");
            if (bk ==1) {
              cli_printf("  %3X    %3d    %4d   %s     %2d     EMBED    %04X    * %-25s  @ 0x%08X ", pg, bk, TULIP_Pages.getword(addr + 0x00, bk), 
                                                                                    rev, 
                                                                                    TULIP_Pages.getword(addr + 0x01, bk),
                                                                                    TULIP_Pages.getflags(pg, bk),
                                                                                    TULIP_Pages.Pages[pg].m_banks[bk].b_img_name,
            ...
            // do not print Page number for readability
              cli_printf("         %3d    %4d   %s     %2d     EMBED    %04X    * %-25s  @ 0x%08X ", bk, TULIP_Pages.getword(addr + 0x00, bk), 
                                                                                    rev, 
                                                                                    TULIP_Pages.getword(addr + 0x01, bk),
                                                                                    TULIP_Pages.getflags(pg, bk),
                                                                                    TULIP_Pages.Pages[pg].m_banks[bk].b_img_name,
    ...
    if (! TULIP_Pages.isPlugged(p, b)) {
      ...
      if (flags & PAGE_RESERVED) {
        // page is reserved, so show the stored filename
        TULIP_Pages.getFileName(p, b, FileNm);           // get the file name of the reserved page
        cli_printf("  reserved for: %s", FileNm);
      }
      return; 
    }

    // print all flags and their meanings for the given page and bank

    cli_printf("  Page %0X Bank %d details, file: %s", p, b, TULIP_Pages.Pages[p].m_banks[b].b_img_name);
    cli_printf("  Flags: %04X", flags);
    // show the flag status
    if (flags & PAGE_EMBEDDED) {
      cli_printf("  Page is EMBEDDED in firmware");
    } else if (flags & PAGE_FLASH) {
      cli_printf("  Page in FLASH");
    } else {
      cli_printf("  Page in FRAM");
    }

    if (flags & PAGE_ROM) {
      cli_printf("       ROM file");
    } else if (flags & PAGE_MOD) {
      cli_printf("       MOD1 file");
    } else {
      cli_printf("       MOD2 file");
    }

    if (flags & PAGE_ENABLED) {
      cli_printf("       ENABLED for reading");
    } else {
      cli_printf("       DISABLED for reading");
    }

    if (flags & PAGE_WRITEABLE) {
      cli_printf("       WRITE ENABLED");
    } else {
      cli_printf("       WRITE PROTECTED");
    }
    ...
    if (flags & PAGE_POLLING) {
      cli_printf("       POLLING entries xFF4..xFFA disabled ");
    }
    ...
    char  ShowPrint[250];
    int   ShowPrintLen = 0;
    uint32_t addr = p * 0x1000; // address of the page
...
void uif_delete(const char *fname)
{
  ...
  uint32_t offs_qr = fr_findfile(fname);

  if ((offs == NOTFOUND) && (offs_qr == NOTFOUND))   {
    cli_printf("  file \"%s\" not found in FLASH/FRAM", fname);
    return;
  }
  ...
  if (offs != NOTFOUND) {
    ...
    // file in FRAM, so delete it
    
    // copy the file header from FRAM to a buffer
    ModuleMetaHeader_t MetaH; 
    fr_read(offs_qr, (uint8_t*)&MetaH, sizeof(ModuleMetaHeader_t));       // read header from FRAM to struct

    cli_printf("  deleting file \"%s\" from FRAM", fname);
    cli_printf("  DO NOT FORGET TO UNPLUG THE PAGE IF IT IS PLUGGED");
    cli_printf("  filename                         type      size  address     next file");
    cli_printf("  -------------------------------  ----  --------  ----------  ----------");
    cli_printf("  %-31s  0x%02X  %8d  0x%08X  0x%08X", 
              MetaH.FileName, MetaH.FileType, MetaH.FileSize, offs_qr, MetaH.NextFile); 

    // delete the file

    uint8_t value = FILETYPE_DELETED; // value to write to mark the file as deleted, can be any value that is not a valid file type
    fr_write(offs_qr, &value, 1); // write the value to the file type byte in the header
    cli_printf("  file \"%s\" marked as deleted", fname);
  }
}
...
void showreg(uint16_t regnum, uint64_t value)
{

  // show the value of the register as individual bytes, separated by spaces, and the register number
  cli_printfn("  REG %03X =  [%02X %02X %02X %02X %02X %02X %02X]", 
            regnum, 
            (uint8_t)(value >> 48) & 0xFF, (uint8_t)(value >> 40) & 0xFF, (uint8_t)(value >> 32) & 0xFF,
            (uint8_t)(value >> 24) & 0xFF, (uint8_t)(value >> 16) & 0xFF, (uint8_t)(value >> 8) & 0xFF, (uint8_t)(value >> 0) & 0xFF);

            
  // show the register contents as ascii characters, non-printable characters are shown as dots
  char ascii[8];
  for (int i = 1; i < 8; i++) {
    uint8_t byte = (value >> ((7 - i) * 8)) & 0xFF;
    if (byte >= 32 && byte <= 126) {
      ascii[i-1] = (char)byte;
    } else {
      ascii[i-1] = '.';
  ...
  // show the register in S.M.SX format
  cli_printf("   %1X.%1X%1X%1X%1X%1X%1X%1X%1X%1x%1X.%1X.%02X ", 
            (uint8_t)(value >> 52) & 0xF,    // mantissa sign
            (uint8_t)(value >> 48) & 0xF, (uint8_t)(value >> 44) & 0xF, (uint8_t)(value >> 40) & 0xF, (uint8_t)(value >> 36) & 0xF, 
            (uint8_t)(value >> 32) & 0xF,   // mantissa
            (uint8_t)(value >> 28) & 0xF, (uint8_t)(value >> 24) & 0xF, (uint8_t)(value >> 20) & 0xF, (uint8_t)(value >> 16) & 0xF, 
            (uint8_t)(value >> 12) & 0xF,   // mantissa
            (uint8_t)(value >> 8) & 0xF,     // exponent sign
            (uint8_t)(value >> 0) & 0xFF);   // exponent
}
...
//   1  command 1 - status
//   2  command 2 - dump
//   3  command 3 - clear XMEM
// 1NM  plug N XFUN memory and M XMEM modules

//uif_xmem(100 + (xmem_N*10 + xmem_M));  
...
void uif_xmem(int i)          
{
  ...
  switch (i) {
    case 1: // status
            cli_printf("  X-Functions Memory [0x040 - 0x0BF]: %s", (globsetting.get(xmem_pages) & xfun_plugged)  ? "plugged" : "not plugged");
            cli_printf("  X-Memory Module #1 [0x201 - 0x2EF]: %s", (globsetting.get(xmem_pages) & xmem1_plugged) ? "plugged" : "not plugged");
            cli_printf("  X-Memory Module #2 [0x301 - 0x3EF]: %s", (globsetting.get(xmem_pages) & xmem2_plugged) ? "plugged" : "not plugged");
            break;
    case 2: // dump XMEM contents, starting at FRAM 0x1E00
            ...
            // only show plugged memory modules
            if ((globsetting.get(xmem_pages) & xfun_plugged) == 0) {
              cli_printf("  X-Functions Memory not plugged, no contents to show");
              // return;
            } else {
              cli_printf("  X-Functions Memory contents from FRAM (non-zero registers only)");
              for (j = 0 ; j < 128; j++) {   // 128 registers
                fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_XMEM0_start + j * 8), (uint8_t*)&reg_cont, 8);
                if (reg_cont != 0) {
                  showreg(0x040 + j , reg_cont);
                }
              }
            }

            if ((globsetting.get(xmem_pages) & xmem1_plugged) == 0) {
              cli_printf("  X-Memory Module #1 not plugged, no contents to show");
              return;
            } else {
              cli_printf("  X-Memory Module #1 contents from FRAM (non-zero registers only)");
              for (j = 0; j < 0x100; j++) {   // 256 registers
                fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_XMEM1_start + j * 8), (uint8_t*)&reg_cont, 8);
                if (reg_cont != 0) {
                  showreg(j + 0x200, reg_cont);
                }
              }
            }

            if ((globsetting.get(xmem_pages) & xmem2_plugged) == 0) {
              cli_printf("  X-Memory Module #2 not plugged, no contents to show");
              return;
            } else {
              cli_printf("  X-Memory Module #2 contents from FRAM (non-zero registers only)");
              for (j = 0; j < 0x100; j++) {   // 256 registers
                fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_XMEM2_start + j * 8), (uint8_t*)&reg_cont, 8);
                if (reg_cont != 0) {
                  showreg(j + 0x300, reg_cont);
                }
              }
            }

            break;           
    case 3: // erase all XMEM, plugged or unplugged, by setting all registers to 0
            reg_cont = 0;      // set reg_cont to 0
            for (j = 0; j < (0x200 + 128); j++) {
              fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_XMEM0_start + j * 8), (uint8_t*)&reg_cont, 8);
            }
            cli_printf("  all Extended Memory registers set to 0");
            break;    


    default:  // i >= 100 means plug memory modules, extract the number of modules from the value
            if (i >= 100) {
              int num_xfun = (i - 100) / 10; // number of XFUN modules to plug
              int num_xmem = (i - 100) % 10; // number of XMEM modules to plug
              if (num_xfun > 1 || num_xmem > 2) {
                cli_printf("  invalid value for plugging memory modules, maximum is 1 XFUN and 2 XMEM modules");
                return;
              }
              globsetting.set(xmem_pages, (num_xfun > 0 ? xfun_plugged  : 0) | 
                                          (num_xmem > 0 ? xmem1_plugged : 0) | 
                                          (num_xmem > 1 ? xmem2_plugged : 0));
              globsetting.save();
              cli_printf("  %d XFUN module(s) and %d XMEM module(s) plugged", num_xfun, num_xmem);
            } else {
              cli_printf("  invalid argument for Extended Memory control");
            }
  }
}

// functions for contro of User Memory Modules
// values passed:
//   1  - status
//   2  - quad
//   2  - dump
//   3  - erase
// 10N  plug N User Memory Modules  
// #define umem_status     1
// #define umem_quad       2
// #define umem_dump       3
// #define umem_erase      4    
...
void uif_umem(int i) {
  ...
  uint64_t reg_cont;
  int num_mods;
  uint16_t umem_start_reg;
  uint16_t umem_end_reg;

  if (!uif_pwo_low()) return;    // only do this when calc is not running

  if (i == umem_quad) i = 104;   // Quad is the same as plugging 4 Memory Modules, so we can reuse the same code

  switch (i) {
    case umem_status: // status
            cli_printf("  User Memory Modules plugged: %d", globsetting.get(umem_pages));
            cli_printf("  User Memory Module #1 [0x100 - 0x13F]");
            cli_printf("  User Memory Module #2 [0x140 - 0x17F]");
            cli_printf("  User Memory Module #3 [0x180 - 0x1BF]");
            cli_printf("  User Memory Module #4 [0x1C0 - 0x1FF]");
            break;

    case umem_dump: // dump UMEM contents, starting at FRAM 0x1C00
            cli_printf("  User Memory Modules plugged: %d", globsetting.get(umem_pages));
            cli_printf("  User Memory Module #1 [0x100 - 0x13F]");
            cli_printf("  User Memory Module #2 [0x140 - 0x17F]");
            cli_printf("  User Memory Module #3 [0x180 - 0x1BF]");
            ...
            umem_start_reg = 0x100; // starting register number for UMEM
            umem_end_reg = globsetting.get(umem_pages) * 0x40 + 0x100; // end register number for UMEM, depends on the number of plugged modules

            if (globsetting.get(umem_pages) == 0) {
              cli_printf("  no User Memory Modules plugged, no contents to show");
              return;
            } 

            cli_printf("   contents from FRAM (non-zero registers only)");
            
            // only show plugged memory modules
            for (j = umem_start_reg; j < umem_end_reg; j++) {   // 64 registers per module
              fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_QUADMEM_start + (j - 0x100) * 8), (uint8_t*)&reg_cont, 8);
              if (reg_cont != 0) {
                showreg(j, reg_cont);
              }
            }
    ...
    case umem_erase: // erase all UMEM, plugged or unplugged, by setting all registers to 0
            reg_cont = 0;      // set reg_cont to 0
            for (j = 0x100; j < 0x200; j++) {
              fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, (FRAM_QUADMEM_start + (j - 0x100) * 8), (uint8_t*)&reg_cont, 8);
            }
            cli_printf("  all User Memory registers set to 0");
            break;

    case umem_quad: // plug 4 UMEM modules, this is the same as plugging 4 modules, so we can reuse the same code

    default:  // i > 100 means plug memory modules, extract the number of modules from the value
            if (i >= 100) {
              int num_umem = i - 100; // number of UMEM modules to plug
              if (num_umem < 0 || num_umem > 4) {
                cli_printf("  invalid value for plugging User Memory Modules, must be 0..4");
                return;
              }
              globsetting.set(umem_pages, num_umem);
              globsetting.save();
              cli_printf("  User Memory Module(s) plugged: %d", num_umem);
            } else {
              cli_printf("  invalid argument for User Memory control");
            }
  }  // switch (i)
...
void fr_init_filesystem()
{
    ...
    // - TULIP_Config.TCF               TULIP Configuration File, all persistent settings
    // - TULIP_Module.TMM               TULIP Moudle Map file
    // - TULIP_Tracer.TRF               TULIP Tracer filters
    // - User_MModule.UMM               User Memory file
    // - Extd_MModule.XMM               Extended Memory file
    ...
    strcpy(MetaH.FileName, "User_MModule.UMM");
    MetaH.FileSize = FRAM_USERMEM_SIZE; // size of user memory
    MetaH.NextFile = FRAM_EXTDMEM_HEADER;
    fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, FRAM_USERMEM_HEADER, (uint8_t*)&MetaH, sizeof(ModuleMetaHeader_t));
    // write empty user memory
    uint8_t zerobuf[256];
    ...
    strcpy(MetaH.FileName, "Extd_MModule.XMM");
    MetaH.FileSize = FRAM_EXTDMEM_SIZE; // size of extended memory
    MetaH.NextFile = FRAM_FS_CONTENT_START; // next file after this one
    fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, FRAM_EXTDMEM_HEADER, (uint8_t*)&MetaH, sizeof(ModuleMetaHeader_t));
    // write empty extended memory
    ...
    fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, FRAM_FS_CONTENT_START, (uint8_t*)&MetaH.FileType, sizeof(uint8_t));
      ...
      cli_printf("    FRAM_TRACER         0x%05X", FRAM_TRACER_CONTENT);
      cli_printf("    FRAM_STATUS regs    0x%05X", FRAM_STATUS_REG_START);
      cli_printf("    FRAM_USERMEM        0x%05X", FRAM_USERMEM_START);
      cli_printf("    FRAM_QUADMEM        0x%05X", QUADMEM_START);
...
void uif_fram(int i, uint32_t addr) {
  ...
  switch (i) {
    ...
    case fram_dump: // dump
            ...
            fram_show(dump_addr);
            break;
    case fram_init: // init FRAM file system
            cli_printf("  initializing FRAM FILE SYSTEM");
            fr_init_filesystem();
            ...
            cli_printf("  FRAM FILE SYSTEM initialized, settings reset to default values");
            break;

    case fram_nukeall: // nukeall
            cli_printf("  ERASING ALL FRAM in 4 seconds!! press any key to cancel");
            cli_printf("  will unplug all modules and reset all TULIP4041 settings to default");
            while(true) {
              tud_task();                 // to process IO until the watchdog triggers
              sleep_ms(2);
              if (cdc_available(ITF_CONSOLE)) {
              ...
              sleepcount--;
              if (sleepcount == 0) {  
                cli_printf("  NUKING FRAM to all zeros");
                // Erase the FLASH File System when the counter expires
                fr_nukeall();
                cli_printf("  FRAM ERASED");
                cli_printf("  use the FRAM INIT command to initialize the FRAM File system");
...
void uif_wand(int i, const char* fname, char* instruction)
{
  ...
  switch (i) {
    ...
    case wand_list: // list
            ...
            break;
    case wand_send: // send direct WAND instructions from command line, decimal arguments
            if (instruction == NULL || *instruction == '\0') {
              cli_printf("  nothing to send ...");
              break;
            }
            // cli_printf("  instruction found: %s", instruction);
            ...
            wakemeup_41();
            break;  
    case wand_sendx: // send direct WAND instructions from command line, hex arguments
            if (instruction == NULL || *instruction == '\0') {
              cli_printf("  nothing to send ...");
              break;
            }
            ...
            while (instruction != NULL && *instruction != '\0') {
              ...
              // convert the token to a number
              value = (uint8_t)strtol(token, NULL, 16);
              cli_printf("  sending WAND instruction byte: 0x%02X   decimal %d", value, value);
              queue_add_blocking(&WandBuffer, &value);
