...
void fr_nukeall()
{
    // erase all FRAM to 0xFF
    uint8_t buf[1024]; // buffer to write 1K bytes at a time
    memset(buf, 0x00, sizeof(buf));  // buffer to write 1K bytes at a time
    for (uint32_t addr = 0; addr < FRAM_SIZE; addr += 1024) {
      fram_write(SPI_PORT_FRAM, PIN_SPI0_CS, addr, buf, 1024);
      tud_task();
        // fr_write(addr, buf, sizeof(buf));  // write 1K bytes of zero
    }
    cli_printf("  All FRAM erased to 0x00");
}
...
bool fr_isinited()
{
    ...
    uint8_t filetype = 0; 
    fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, FRAM_FS_START, &filetype, 1); // read the byte

    // and read the file name to check if it is a valid file system
    char filename[32];
    fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, FRAM_FS_START + 1, (uint8_t*)filename, sizeof(filename));

    // and compare with the standard filename "TULIP4041 FRAM HEADER" to check if the file system is initialized
    if ((filetype != FILETYPE_4041) || (strncmp(filename, "TULIP4041 FRAM HEADER", 21) != 0)) {
...
{
    uint32_t addr = FRAM_FS_START;     // our address counter, 
                                        // start at the beginning of the file system
    ModuleMetaHeader_t MetaH;           // meta header for getting info of next file

    while (addr < FRAM_SIZE) {
      // read the File Header from FRAM
      fram_read(SPI_PORT_FRAM, PIN_SPI0_CS, addr, (uint8_t*)&MetaH, sizeof(ModuleMetaHeader_t));
      if (MetaH.FileType == FILETYPE_FFFF) {
        return NOTFOUND;                 // end of chain reached, file not found   
      }
