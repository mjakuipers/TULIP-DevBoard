Some ready made MOD files are provided here for convenience. Take care that these configurations may work differently (or not) in non-TULIP environments. 
...
- HEPAX RAM files for use with HEPAX. The MOD files are modified such that these will plug in any Page and may end up in non-consecutive Pages, depending on the current configuration and reserved Pages
...
The CLI offers the ***hepram INIT P*** command to initialize a single page of HEPRAM. Easiest is to use the ***hepram INITALL*** command to initialize all HEPRAM pages.
